...
class CustomAlignedInferenceDataset(CustomAlignedDataset):
    ...
    def __getitem__(self, index):
        ...
        # verts = self.bfm.apply_Rts(verts, R, t, s).astype(np.float)
        # mesh = Meshes([to_tensor(verts, "cuda")], [to_tensor(self.bfm.faces, "cuda").long()])
        #
        # uv = self.render_uv(mesh, item["TARGET"].shape[-2], item["TARGET"].shape[-1])
