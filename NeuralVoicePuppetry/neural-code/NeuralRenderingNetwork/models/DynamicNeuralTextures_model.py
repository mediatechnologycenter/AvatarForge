...
class DynamicNeuralTexturesModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            self.visual_names = ['input_uv', 'features', 'fake', 'target']
        else:
            self.visual_names = ['input_uv', 'fake', 'target', 'source']
        ...
        if self.opt.deca_details:
            self.texture_decoder = define_TextureDecoder(opt.rendererType, opt.tex_features + 6, opt.ngf, opt.norm, not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        else:
            self.texture_decoder = define_TextureDecoder(opt.rendererType, opt.tex_features + 3, opt.ngf, opt.norm,
    ...
    def set_input(self, input):
        ...
        if self.opt.deca_details :
            self.deca_details = input['deca_details'].cuda()
        ...
        if "REFERENCE_POSE" in input:
            self.reference_pose = input["REFERENCE_POSE"].to(self.device)
    ...
    def forward(self, alpha=1.0):
        ...
        if 0:
            fig = plt.figure()
            fig.add_subplot(241)
            plt.imshow(self.input_uv[0,0:1,:,:].permute(1, 2, 0).cpu().numpy())
            fig.add_subplot(242)
            plt.imshow(self.input_uv[0, 1:2, :, :].permute(1, 2, 0).cpu().numpy())
            fig.add_subplot(243)
            plt.imshow(mask[0].permute(1, 2, 0).cpu().numpy().astype(np.float32))
            fig.add_subplot(244)
            plt.imshow((((self.background[0].permute(1, 2, 0).cpu().numpy() + 1.) / 2.) * 255.).astype(np.uint8))
            fig.add_subplot(245)
            plt.imshow((((self.target[0].permute(1, 2, 0).cpu().numpy() + 1.) / 2.) * 255.).astype(np.uint8))
            fig.add_subplot(246)
            plt.imshow(torch.zeros_like(self.target)[0].permute(1, 2, 0).cpu().numpy())
            plt.show()
        ...
        for b in range(0,batch_size):
            ...
            if self.opt.deca_details:
                feat = torch.cat((feat, self.deca_details[b:b+1]), dim=1)
    ...
    def backward_D(self):
        ...
        if self.opt.deca_details:
            fake_AB = self.fake_AB_pool.query(torch.cat((self.input_uv, self.deca_details, masked(self.fake)), 1))
        ...
        # Real
        if self.opt.deca_details:
            real_AB = torch.cat((self.input_uv, self.deca_details, masked(self.target)), 1)
    ...
    def backward_G(self, epoch):
        ...
        if self.opt.deca_details:
            fake_AB = torch.cat((self.input_uv, self.deca_details, masked(self.fake)), 1)
