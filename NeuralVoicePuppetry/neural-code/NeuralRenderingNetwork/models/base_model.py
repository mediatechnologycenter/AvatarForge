...
class BaseModel():
    ...
    # clean checkpoints
    def clean_checkpoints(self, total_epochs):
        for name in self.model_names:
            if isinstance(name, str):
                for epoch in range(total_epochs):
                    delete_filename = '%s_%s.pth' % (epoch, name)
                    save_path = os.path.join(self.save_dir, delete_filename)
                    if os.path.isfile(save_path):
                        os.remove(save_path)

        shutil.rmtree(os.path.join(self.save_dir, 'web'))
    ...
    def load_networks(self, epoch):
        for name in self.model_names:
            if isinstance(name, str):
                ...
                net = getattr(self, name)
                net_loaded = torch.load(load_path, map_location=str(self.device))
                net.load_state_dict(net_loaded)


                # print('loading the tensor from %s' % load_path)
                # net_loaded = torch.load(load_path, map_location=str(self.device))

                # for i, k in zip(net.state_dict().keys(), net_loaded.keys()):
                #     print(i, k)
                #
                #     net.state_dict()[i] = net_loaded[k]
