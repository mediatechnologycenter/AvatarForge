...
SUB=Halbtotale_355_9415

DATASETS_DIR=$DP/$SUB
DATASET_MODE=custom_aligned
OBJECT=Halbtotale_355_9415.h5
...
NAME=$MODEL-$RENDERER_TYPE-$TEXTUREMODEL-SL$SEQ_LEN-BS$BATCH_SIZE-$OBJECT-$DATASET_MODE-$LOSS-$DATE_WITH_TIME-look_ahead
...
echo "---------"
CUDA_VISIBLE_DEVICES=0 python train_renderer.py --textureModel $TEXTUREMODEL --num_threads $NUM_THREADS --input_nc $INPUT_NC --display_id $DISPLAY_ID --look_ahead --seq_len $SEQ_LEN  --save_latest_freq 100000 --no_augmentation --name $NAME --erosionFactor $EROSION --tex_dim $TEX_DIM --tex_features $TEX_FEATURES --rendererType $RENDERER_TYPE --lossType $LOSS --display_env $DISPLAY_NAME --niter $N_ITER --niter_decay $N_ITER_LR_DECAY --dataroot $DATASETS_DIR/$OBJECT --model $MODEL --netG unet_256 --lambda_L1 100 --dataset_mode $DATASET_MODE --no_lsgan --norm instance --pool_size 0  --gpu_ids $GPUID --lr $LR --batch_size $BATCH_SIZE
