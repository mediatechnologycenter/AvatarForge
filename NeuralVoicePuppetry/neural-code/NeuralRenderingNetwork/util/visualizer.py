...
def imresize(arr, shape, method):
    return numpy.array(Image.fromarray(arr).resize(shape[::-1], method))
...
class Visualizer():
    def __init__(self, opt):
        ...
        if self.use_html:
            self.web_dir = os.path.join(opt.checkpoints_dir, opt.name, 'web', opt.phase)
            if "images_target_dir" in opt:
                self.img_dir = os.path.join(opt.images_target_dir, 'images')
            else:
        ...
        if self.use_html and (save_result or not self.saved):  # save images to a html file
            ...
            for label, image in visuals.items():
                if label == "features":
                    image = image[:, :3]
                image_numpy = util.tensor2im(image)
                img_path = os.path.join(self.img_dir, 'epoch%.5d_%s.png' % (epoch, label))
                util.save_image(image_numpy, img_path)
            # update website
            webpage = ownhtml.OwnHTML(self.web_dir, 'Experiment name = %s' % self.name, reflesh=1)
            for n in range(epoch, 0, -1):
                ...
                for label, image_numpy in visuals.items():
                    image_numpy = util.tensor2im(image)
                    img_path = 'epoch%.5d_%s.png' % (n, label)
                    ims.append(img_path)
                    txts.append(label)
...
class InferenceVisualizer(Visualizer):
    def display_current_results(self, visuals, epoch, save_result, aspect_ratio=1.0, width=256):
        image = visuals['fake']
        image_numpy = util.tensor2im(image)
        img_path = os.path.join(self.img_dir, '%05d.png' %epoch)
        util.save_image(image_numpy, img_path)