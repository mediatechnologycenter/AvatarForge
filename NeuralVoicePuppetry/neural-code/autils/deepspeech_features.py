...
def extract_ds(folder_videos, file_id, target_base, target_name, type, target_fps):
    ...
    if type == 'video':
        ...
        if not os.path.isfile(path_wav):
            ...
            if not os.path.isfile(path_video_in):
                path_video_in = '{}/{}.avi'.format(folder_videos, file_id)

            # cmd = "ffmpeg -i input-video.avi -vn -acodec copy output-audio.aac"

            cmd = ('ffmpeg' + f' -i {path_video_in} {path_wav}').split()
            call(cmd)
        ...
        if not os.path.isfile(path_wav):
            try:
                path_mp3 = '{}/{}.mp3'.format(folder_videos, file_id)
    ...
    folder_out = os.path.join(target_base, target_name, "audio_feature")

    # Save features
    mkdir(folder_out)
    for i, feature in enumerate(features):
        fn_out = "{}.deepspeech.npy".format(i)
