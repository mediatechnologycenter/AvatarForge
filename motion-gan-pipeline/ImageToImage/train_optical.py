...
from utils.optical_flow import backwarp
...
def config_parser():
    ...
    parser.add_argument("--flow_train_root_dir", type=str, help="Path to training optical flows", default='./data/opticalflow_train') 
    ...
    parser.add_argument("--flow_val_root_dir", type=str, help="Path to val optical flows", default='./data/opticalflow_val') 
    ...
    parser.add_argument("--optical_steps", type=int, default=1, help="Number of stept to go back with optical flow")
...
if __name__=='__main__':
    ...
    for epoch in range(args.continue_from_epoch, args.num_epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            # Loss on: generated VS target
            loss_perceptual = loss_fn(prediction, labels)

            # Loss on: warped generated with opticalflow VS previous target frame
            loss_temporal = torch.zeros_like(loss_perceptual)

            for flow, prev in zip(flows, prevs):
                flow = flow.to(device)
                prev = prev.to(device)
                warped_prediction = backwarp(prediction, flow)
                loss_temporal += loss_fn(warped_prediction, prev)
            ...
            if args.use_discriminator and epoch>start_train_disc:
                ...
                loss = loss_perceptual + loss_temporal + loss_gan
            else:
                loss = loss_perceptual + loss_temporal
            ...
            if batch_idx % args.log_every == 0:
                if args.save_val_images:   
                    image_concat = create_image_pair([inputs.detach(), prediction.detach(), labels.detach(), warped_prediction.detach(), prev.detach()])
                    wandb_images = [wandb.Image(image) for image in image_concat]
                    if args.use_discriminator and epoch>start_train_disc:
                        wandb.log({'train loss perceptual': loss_perceptual, 
                                   'train loss temporal': loss_temporal, 
                        ...
                        wandb.log({'train loss perceptual': loss_perceptual, 
                                   'train loss temporal': loss_temporal}, step=step)         
                        wandb.log({'train_loss': loss.detach()}, step=step)
                else:
                    if args.use_discriminator and epoch>start_train_disc:
                        wandb.log({'train loss perceptual': loss_perceptual, 
