...
# def compute_mel_one_sequence(audio, sr=16000, fps=60, device='cpu'):
#     ''' compute mel for an audio sequence. 
#     '''
    
    
#     hop_length=int(sr/(fps*2))
#     win_length=1/fps
#     winlen = int(sr/fps)
#     winstep=0.5/fps
...
#     Audio2Mel_torch = audio_funcs.Audio2Mel(n_fft=n_fft,
...
#     mel_nframe = nframe
#     mel_frame_len = int(sr * win_length)
...
#         mel_tmp = Audio2Mel_torch(audio_clip_device).cpu().numpy()
#         mel80s[i] = mel_tmp[0].T  # [1, 80] -> we get [2, 80] ???
#         print(mel80s[i].size)
...
def headpose_smooth(headpose, smooth_sigmas=[0,0], method='gaussian'):
    ...
    if method == 'gaussian':
        ...
        rot = median_filter(headpose.reshape(-1, 6)[:,:3], size=rot_sigma).reshape(-1, 3)
        trans = median_filter(headpose.reshape(-1, 6)[:,3:], size=trans_sigma).reshape(-1, 3)
        headpose_smooth = np.concatenate([rot, trans], axis=1)

    else:
        print(f'{method} smoothing not implemented.')
        return headpose
