...
class Audio2Headpose(nn.Module):
    def __init__(self, opt):
        ...
        self.audio_downsample = nn.Sequential(
                        nn.Linear(in_features=opt.APC_hidden_size, out_features=opt.APC_hidden_size),
                        nn.BatchNorm1d(opt.APC_hidden_size),
                        nn.LeakyReLU(0.2),
                        nn.Linear(opt.APC_hidden_size, opt.APC_hidden_size),
                        )
        
        self.WaveNet = WaveNet(residual_layers=opt.A2H_wavenet_residual_layers,
                               residual_blocks=opt.A2H_wavenet_residual_blocks,
                               dilation_channels=opt.A2H_wavenet_residual_channels,
                               residual_channels =opt.A2H_wavenet_dilation_channels,
                               skip_channels=opt.A2H_wavenet_skip_channels,
                               kernel_size=opt.A2H_wavenet_kernel_size,
                               output_length=opt.time_frame_length,
                               use_bias=opt.A2H_wavenet_use_bias,
                               cond=True,
                               input_channels=opt.A2H_wavenet_input_channels,
                               ncenter=opt.A2H_GMM_ncenter,
                               ndim=opt.A2H_GMM_ndim,
                               output_channels=output_size,
                               cond_channels=opt.A2H_wavenet_cond_channels)
