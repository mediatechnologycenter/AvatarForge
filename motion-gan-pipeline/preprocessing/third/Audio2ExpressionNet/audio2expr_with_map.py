...
from autil.visualizer import save_images
from autil import html
import torch
...
if __name__ == '__main__':
    ...
    audio2expr_dir = os.path.join(out_dir, 'audio_expr')
    ...
    for i, data in tqdm(enumerate(data_loader_source)):
        ...
        if opt.use_mapping:
            ...
            # np.save(os.path.join(audio2expr_dir, f'expr_{i}.npy'), expression.cpu().numpy())
            np.save(os.path.join(audio2expr_dir, 'audio_expression_%05d.npy' % i), expression.cpu().numpy())
